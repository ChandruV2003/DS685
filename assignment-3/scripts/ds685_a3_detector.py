...
class DetectorPublisher(Node):
    def __init__(self) -> None:
        super().__init__("ds685_a3_detector")
    ...
    def _lookup_map_pose(self, stamp: Time) -> tuple[bool, float | None, float | None, float | None]:
        try:
            tf = self._tf_buffer.lookup_transform(
                target_frame="map",
                source_frame=self.base_frame,
            ...
            t = tf.transform.translation
            q = tf.transform.rotation
            return True, float(t.x), float(t.y), _q_to_yaw(q.x, q.y, q.z, q.w)
        except Exception:
    ...
    def _maybe_save_image(self, sha256: str, cv_img: Any, has_dets: bool) -> None:
        if not self.save_images:
            return
        if (not self.save_all_images) and (not has_dets):
            return
        out = self.image_out_dir / f"{sha256}.jpg"
        if out.exists():
            return
        try:
            import cv2
            ...
            self.get_logger().warning(f"Failed to save image {out}: {e}")
    ...
    def _on_image(self, msg: Image) -> None:
        ...
        map_ok, map_x, map_y, map_yaw = self._lookup_map_pose(stamp)
        ...
        self._maybe_save_image(sha256, cv_img, has_dets=bool(det_list))
