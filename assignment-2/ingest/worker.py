...
def _zenoh_open(connect_endpoint: str) -> zenoh.Session:
    cfg = zenoh.Config.from_json5(
        json.dumps({"connect": {"endpoints": [connect_endpoint]}})
    )
...
def insert_event(conn: psycopg.Connection, event: dict[str, Any]) -> None:
    ...
    # NOTE(psycopg3): `with conn:` closes the connection on exit.
    # Use an explicit transaction context instead.
    ...
    def handler(sample: zenoh.Sample) -> None:
        try:
            ...
            try:
                ...
                logging.warning("Dropping event due to full ingest queue (key=%s)", getattr(sample, "key_expr", None))
        except Exception:
