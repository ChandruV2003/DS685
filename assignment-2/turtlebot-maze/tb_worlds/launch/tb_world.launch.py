...
def generate_launch_description():
    ...
    declare_headless_cmd = DeclareLaunchArgument(
        "headless",
        default_value="False",
        description="Run Gazebo server-only with headless rendering",
    )
    ...
    # Ensure Gazebo can resolve model:// URIs for tb_worlds-provided models.
    set_env_tb_world_models = AppendEnvironmentVariable(
        "GZ_SIM_RESOURCE_PATH", os.path.join(bringup_dir, "models")
    )
    set_env_tb_world_share = AppendEnvironmentVariable("GZ_SIM_RESOURCE_PATH", bringup_dir)
    ...
    gazebo_headless = ExecuteProcess(
        ...
        cmd=["gz", "sim", "-r", "-s", "--headless-rendering", world_sdf],
        output="screen",
    )
    ...
    return LaunchDescription(
        [
            ...
            set_env_tb_world_models,
            set_env_tb_world_share,
            world_sdf_xacro,
